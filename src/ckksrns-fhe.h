...
#ifndef FRNS_BOOTSTRAP_MYCKKSBOOTSTRAPPRECOM_H
#define FRNS_BOOTSTRAP_MYCKKSBOOTSTRAPPRECOM_H
...
std::vector<Plaintext> EvalLinearTransformPrecompute(
        Scheme scheme, const std::vector<std::vector<std::complex<double>>>& A, double scale, uint32_t L);

//fixme: change vector to array!
...
//    std::vector<Plaintext> m_U0Pre;
    Plaintext *m_U0Pre;

    // Conj(U0^T); used in encoding
//    std::vector<Plaintext> m_U0hatTPre;
    Plaintext *m_U0hatTPre;

    // coefficients corresponding to U0; used in decoding
//    std::vector<std::vector<Plaintext>> m_U0PreFFT;
    Plaintext **m_U0PreFFT;

    // coefficients corresponding to conj(U0^T); used in encoding
//    std::vector<std::vector<Plaintext>> m_U0hatTPreFFT;
    Plaintext **m_U0hatTPreFFT;
};
...
static uint32_t m_correctionFactor = 0;  // correction factor, which we scale the message by to improve precision

// Chebyshev series coefficients for the SPARSE case
//const std::vector<double> g_coefficientsSparse{
const double g_coefficientsSparse[]={
...
const double g_coefficientsUniform[]={
...
#endif //FRNS_BOOTSTRAP_MYCKKSBOOTSTRAPPRECOM_H
