...
public:
	 ...
	 * @param[in] prefix: string prefix
	 */
	static void showcompare(complex<double>* vals1, complex<double>* vals2, long size, string prefix, bool PRT=0);
    ...
    static void CompareAndSet(long LimbsNum, long N, uint64_t *val, uint64_t * golden_val, string STRING,
                              bool ComparePRT=0, bool Set=0);

    static void CompareAndSet(long LimbsNum, long N, Ciphertext& ct, uint64_t *golden_val, string STRING,
                              bool ComparePRT=0, bool Set=0);

    static void DecAndShow(Scheme scheme, Ciphertext ct, string String);

    static void FullDecAndShow(Scheme scheme, Ciphertext ct, string String);
};
